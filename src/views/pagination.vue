    ...
    <section v-else class="container">
        <ul :class="{ Repocontainer: true }">
            <li v-for="repo in displayed[currentPosition - 1]" :key="repo.id">
                <RouterLink :to="{ path: `/Orezy01/repo/${repo.name}`, params: true }">
                    <button type="button" class="btn btn-outline-primary">{{ repo.name.toUpperCase() }}</button>
                </RouterLink>
            </li>
        </ul>
        ...
        <section :class="{ pagination: true }">
            <ul>
                ...
                <li v-if="currentPosition < displayed.length" :class="{ next: true }"><button @click="Next"
                         class="btn btn-light">{{ Nextbtn }}</button>
                </li>
            </ul>
        </section>
